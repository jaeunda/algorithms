...
2026년 1월 9일 21:26:27

### Approach
1. 연속된 파일을 합쳐야 하므로 구간 DP로 접근
2. `dp[i][j]`를 `[i, j]`를 합치는 최소 비용으로 간주
3. 구간을 `k` 기준으로 나누어 계산 (`i <= k < j`)
```
dp[i][j] = min over k (i <= k < j)
           dp[i][k] + dp[k+1][j] + sum[j] - sum[i-1]
(이미 계산된) 왼쪽 비용 + 오른쪽 비용 +    합치는 비용
```
- (이미 계산된 왼쪽 구간 비용) + (이미 계산된 오른쪽 구간 비용) + (구간 [i, j] 전체 파일 크기의 합)
4. 구간 길이(`len`)를 늘려가며 DP 테이블 채움
![tempFileForShare_20260109-214726](https://github.com/user-attachments/assets/3b1e846d-875f-446e-8891-f65ffdd106b4)

### Pitfall
- 파일의 크기와 파일을 합치는 비용을 혼동하여 구간 `[i, j]`를 하나로 합칠 때 누적합을 포함하지 않아 초기 로직이 잘못되었음.
