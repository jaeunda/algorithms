...
int main(){
    ...
    int N, M, S, E;
    int input[2001] = {0};
    bool pal_dp[2001][2001] = {0};

    cin >> N;
    for (int i=1; i<=N; i++) cin >> input[i];

    // dp
    // len = 1: pal_dp[i][i] = true;
    // input의 길이가 홀수인 경우
    for (int i=1; i<=N; i++)
        pal_dp[i][i] = true;
    ...
    // input의 길이가 짝수인 경우
    for (int i=1; i<N; i++)
    //  pal_dp[i][i+1] = (input[i] == input[i+1])? true : false;
        pal_dp[i][i+1] = (input[i] == input[i+1]);
    ...
    for (int len=3; len<=N; len++){ // [s,e] 길이
        for (int s=1; s+len-1<=N; s++){ // s: 시작점
            int e = s+len-1; // e: 끝점
            pal_dp[s][e] = (input[s] == input[e]) && pal_dp[s+1][e-1];
        }
    }
